...
async def cb_schedule_custom(query: CallbackQuery, state: FSMContext, user: User) -> None:
    ...
    await query.message.answer(t(user.language, "settings.schedule.custom_prompt"))
    await query.answer()
...
async def cb_flavor(query: CallbackQuery, user: User) -> None:
    assert query.message is not None
    await query.message.answer(
        t(user.language, "settings.flavor.prompt"), reply_markup=flavor_keyboard(user.language)
    )
    await query.answer()
...
async def cb_flavor_set(query: CallbackQuery, callback_data: FlavorCb, user: User) -> None:
    ...
    await query.message.answer(
        t(user.language, "settings.flavor.saved", title=title), reply_markup=main_menu(user.language)
    )
    await query.answer()
...
async def cb_interval(query: CallbackQuery, user: User) -> None:
    assert query.message is not None
    await query.message.answer(
        t(user.language, "settings.interval.prompt"), reply_markup=interval_keyboard(user.language)
    )
    await query.answer()
...
async def cb_toggle(query: CallbackQuery, user: User) -> None:
    ...
    await query.message.answer(t(user.language, key), reply_markup=main_menu(user.language))
    await query.answer()
...
async def cb_language(query: CallbackQuery, user: User) -> None:
    assert query.message is not None
    await query.message.answer(t(user.language, "settings.language.prompt"), reply_markup=language_keyboard())
    await query.answer()
...
async def cb_quiet_set(query: CallbackQuery, callback_data: QuietCb, user: User) -> None:
    ...
    if callback_data.value == "off":
        ...
        await query.message.answer(
            t(user.language, "settings.quiet.off"), reply_markup=main_menu(user.language)
        )
    else:
